...
export const GeneratedGallery: React.FC<GeneratedGalleryProps> = ({ images, isLoading, loadingMessage, onImageClick, onStartNew, projectId, projectName, onSaveToProject }) => {
  if (isLoading) {
    return (
      <section className="py-20 text-center">
        ...
        <h2 className="text-3xl font-bold mb-2 text-white">AI is creating your photoshoot...</h2>
  ...
  return (
    <section className="py-20">
      <div className="text-center mb-12">
        <h2 className="text-4xl font-bold bg-gradient-to-b from-white to-gray-400 bg-clip-text text-transparent">Your AI-Generated Photoshoot</h2>
        <p className="text-lg text-gray-400 mt-2">Click on an image to view it in full screen, or start over.</p>
        <div className="mt-6 flex items-center justify-center gap-4">
          {onSaveToProject && (
            <Tooltip tip={projectName ? `Save to ${projectName}` : 'Save to Project'}>
              <Button onClick={onSaveToProject} variant="primary">
          ...
          <Tooltip tip="Clear current session and start over">
            ...
            <Card className="group" onClick={() => onImageClick(index)}>
              ...
              <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4">
                ...
                <div className="flex items-center text-white font-semibold">
                   <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-nutshel-blue" viewBox="0 0 20 20" fill="currentColor">
