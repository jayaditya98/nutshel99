...
import { HomeIcon, CompassIcon, FolderIcon, PlusIcon, SettingsIcon } from './WorkspaceIcons';
...
const BottomNavBar: React.FC = () => {
  ...
  const navItems: Omit<NavLinkItem, 'label'>[] = [
    { path: '/workspace/dashboard', icon: <HomeIcon className="w-6 h-6" /> },
    { path: '/workspace/explore', icon: <CompassIcon className="w-6 h-6" /> },
    { path: '/workspace/creations', icon: <FolderIcon className="w-6 h-6" /> },
    { path: '/workspace/settings', icon: <SettingsIcon className="w-6 h-6" /> },
  ];

  const handleCreateClick = () => {
    navigate('/workspace/creations', { state: { createNew: true } });
  };
