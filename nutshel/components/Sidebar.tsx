...
import { HomeIcon, CompassIcon, GridIcon, FolderIcon, FilmIcon, PenToolIcon, LayersIcon, PlusIcon, ChevronsLeftIcon, ChevronsRightIcon, ZapIcon, SettingsIcon, LogOutIcon } from './WorkspaceIcons';
...
const Sidebar: React.FC = () => {
  ...
  const primaryNav: NavLinkItem[] = [
    { path: '/workspace/canvas', label: 'AI Canvas', icon: <PenToolIcon className="w-5 h-5" /> },
    { path: '/workspace/studios', label: 'Nutshel Studios', icon: <FilmIcon className="w-5 h-5" /> },
    { path: '/workspace/suite', label: 'Edit Images', icon: <LayersIcon className="w-5 h-5" /> },
  ];
  
  const secondaryNav: NavLinkItem[] = [
    { path: '/workspace/creations', label: 'All Creations', icon: <FolderIcon className="w-5 h-5" /> },
    { path: '/workspace/explore', label: 'Explore', icon: <CompassIcon className="w-5 h-5" /> },
  ];
    ...
    <div className={`bg-nutshel-gray flex-shrink-0 p-4 flex flex-col justify-between hidden md:flex transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'}`}>
      <div>
        <div className={`flex items-center mb-8 ${isCollapsed ? 'flex-col items-center space-y-4' : 'justify-between'}`}>
            <Link to="/workspace/dashboard" className="flex items-center gap-2 text-white opacity-90 hover:opacity-100" title="Dashboard">
            ...
            <div className="relative" ref={profileRef}>
                ...
                {isProfileOpen && !isCollapsed && (
                   <div className="absolute bottom-full left-0 mb-2 w-full bg-nutshel-gray-dark border border-white/10 rounded-lg shadow-xl z-10 text-sm">
                      ...
                      <div className="p-2">
                          <Link to="/workspace/settings" onClick={() => setIsProfileOpen(false)} className="flex items-center space-x-2 px-2 py-1.5 rounded-md hover:bg-white/10">
