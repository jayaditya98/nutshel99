...
const SuiteToolCard: React.FC<{ title: string; description: string; }> = ({ title, description }) => (
    <div className="bg-nutshel-gray rounded-xl p-6 transition-all duration-300 ease-in-out border border-white/10 hover:-translate-y-1 hover:shadow-lg hover:shadow-nutshel-accent/20 hover:border-white/20 h-full flex flex-col justify-between">
      ...
      <Link to="/workspace/suite" className="text-sm font-semibold text-nutshel-accent mt-4 inline-block hover:underline">Learn more →</Link>
    </div>
);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {recentProjects.map((project, index) => (
            <AnimatedItem key={project.id} delay={index * 100}>
              <Link to={`/workspace/project/${project.id}`} className="block bg-nutshel-gray rounded-xl p-4 transition-all duration-300 ease-in-out border border-white/10 hover:-translate-y-1 hover:shadow-lg hover:shadow-nutshel-accent/20 hover:border-white/20 h-full">
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {studioFeatures.map((feature, index) => {
            ...
            if (feature.title === 'Product Photoshoots') {
              route = '/workspace/product-photoshoots';
            } else if (feature.title === 'Model Shoots') {
              route = '/workspace/model-shoots';
            } else if (feature.title === 'Clone Shoots') {
              route = '/workspace/clone-shoots';
            } else if (feature.title === 'Image Composer') {
              route = '/workspace/image-composer';
            }
            return (
              <AnimatedItem key={feature.title} delay={index * 100}>
                <Link to={route} className="h-full block">
