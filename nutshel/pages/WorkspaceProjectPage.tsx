...
import { PencilIcon, TrashIcon, PlusIcon, EditIcon, PenToolIcon, FilmIcon, FolderIcon } from '../components/WorkspaceIcons';
import { AnimatedItem } from '../components/AnimatedItem';
import AddAssetModal from '../components/AddAssetModal';
import { getStudioImageMetadata } from '../utils/imageStorage';
...
const AssetCard: React.FC<{
    ...
    const handleEdit = async () => {
        if (asset.type === 'ai-canvas') {
            // Open in AI Canvas with saved state
            navigate('/workspace/canvas', { state: { assetSeed: asset.seed, assetName: asset.name } });
        } else if (asset.type === 'nutshel-studios' && asset.sourceApp) {
            try {
                // Get metadata from IndexedDB (where historyId is stored)
                const metadata = await getStudioImageMetadata(asset.seed) || {};
                const historyId = metadata.historyId;
                
                // Map sourceApp to route
                const routeMap: Record<string, string> = {
                    'product-photoshoots': '/workspace/product-photoshoots',
                    'model-shoots': '/workspace/model-shoots',
                    'clone-shoots': '/workspace/clone-shoots',
                    'image-composer': '/workspace/image-composer',
                };
                
                const route = routeMap[asset.sourceApp] || '/workspace/studios';
                
                // Navigate with historyId - this will trigger automatic restoration
                // Equivalent to clicking the restore button in history sidebar
                navigate(route, { 
                    state: { 
                        assetSeed: asset.seed, 
                        metadata,
                        historyId // This will trigger automatic history restoration
                    } 
                });
            } catch (error) {
                console.error('Error loading metadata for re-edit:', error);
                // Fallback: try localStorage for backward compatibility
                try {
                    const metadataStr = localStorage.getItem(`studio_metadata_${asset.seed}`);
                    ...
                    console.error('Error in fallback metadata loading:', fallbackError);
                    // Last resort: navigate without historyId
                    const routeMap: Record<string, string> = {
                        'product-photoshoots': '/workspace/product-photoshoots',
                        'model-shoots': '/workspace/model-shoots',
                        'clone-shoots': '/workspace/clone-shoots',
    ...
    const getAssetImageUrl = (): string | null => {
        if (asset.type === 'ai-canvas') {
        ...
        <div className="group relative">
            <div className="aspect-square bg-nutshel-gray rounded-lg overflow-hidden border border-white/10 transform group-hover:-translate-y-1 transition-transform">
                {imageUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-nutshel-gray-dark">
                        {asset.type === 'ai-canvas' ? (
                            <PenToolIcon className="w-12 h-12 text-gray-600" />
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    {(asset.type === 'ai-canvas' || asset.type === 'nutshel-studios') && (
                        <button 
                            onClick={handleEdit} 
                            className="p-3 bg-white/10 rounded-full text-white hover:bg-white/20 transition-colors" 
                            title={asset.type === 'ai-canvas' ? 'Edit in Canvas' : 'Re-edit'}
                        >
    ...
    if (!project) {
        return (
            <div className="text-center py-20">
                ...
                <Link to="/workspace/creations" className="bg-nutshel-accent text-black font-semibold py-2.5 px-6 rounded-lg hover:opacity-90 transition-opacity">
        ...
        <div className="space-y-12">
            <header className="relative h-64 md:h-80 rounded-2xl overflow-hidden border border-white/10">
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end">
                     <Link to="/workspace/creations" className="absolute top-6 left-6 flex items-center gap-2 text-sm text-white bg-black/30 backdrop-blur-sm py-1.5 px-3 rounded-full hover:bg-black/50 transition-colors">
